...
const { colToIndex, buildClearCellRequests, STANDARD_CLEAR_COLUMNS } = require('../../utils/sheetHelper');
...
async function _executeMoveMember(userId) {
    try {
        ...
        if (memberDataArray && foundRowIndexInPD !== -1) {
            ...
            // ✅ ใช้ shared buildClearCellRequests
            const clearRanges = STANDARD_CLEAR_COLUMNS.map(col => ({
                range: `${sheetName}!${col}${foundRowIndexInPD}`
            }));

            const requests = buildClearCellRequests(foundRowIndexInPD, STANDARD_CLEAR_COLUMNS);
            ...
            try {
                ...
                for (const col of STANDARD_CLEAR_COLUMNS) {
...
// ✅ ใช้ colToIndex จาก sheetHelper แล้ว (getColumnIndex ลบเพราะซ้ำ)
// =================================================================
