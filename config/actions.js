...
async function handleCountSave(interaction) {
    const raw = interaction.fields.getTextInputValue('input_all_channels').split(',').map(s => s.trim());
...
// --- บันทึก: BYPD + Proctor + LogCase ---
async function handleBypdSave(interaction) {
    await sheetConfig.writeConfigKeys([
        ['LOGCASE_CHANNEL_ID',    interaction.fields.getTextInputValue('input_logcase_channel').trim()],
        ['BYPD_SEND_CHANNEL_ID',  interaction.fields.getTextInputValue('input_bypd_send').trim()],
        ['PROCTOR_CHANNEL_ID',    interaction.fields.getTextInputValue('input_proctor_channel').trim()]
    ]);
    return '✅ บันทึกตั้งค่า LogCase + BYPD + Proctor แล้ว';
}
