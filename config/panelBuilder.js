...
function createPanelEmbed() {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            ...
            '**BYPD + Proctor**\n' +
            `• LogCase: ${ch(sheetConfig.getLogCaseChannelId())}\n` +
            `• ส่ง BYPD: ${ch(sheetConfig.getBypdSendChannelId())}\n` +
            `• ส่ง Proctor: ${ch(sheetConfig.getProctorChannelId())}\n\n` +
            '**ชีตลงทะเบียน PD**\n' +
...
function buildPanelComponents(guildId) {
    ...
    return [
        ...
        new ActionRowBuilder().addComponents(
            btn('btn_cfg_bypd',      'ตั้งค่า — ระบบคดี',    ButtonStyle.Secondary, '🆔'),
