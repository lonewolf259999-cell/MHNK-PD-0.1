...
function buildBypdModal() {
    return createModalBase('modal_cfg_bypd', 'ตั้งค่า — BYPD + Proctor + LogCase', [
        ['input_logcase_channel', 'LOGCASE_CHANNEL_ID (ห้องรวม BYPD+Proctor)',  sheetConfig.getLogCaseChannelId()],
        ['input_bypd_send',       'BYPD_SEND_CHANNEL_ID (ส่ง BYPD ไปที่)',       sheetConfig.getBypdSendChannelId()],
        ['input_proctor_channel', 'PROCTOR_CHANNEL_ID (ส่ง Proctor ไปที่)',       sheetConfig.getProctorChannelId()]
    ]);
}
