...
function buildViews(data) {
    const result = {
        ...
        logCaseChannelId: data.LOGCASE_CHANNEL_ID || DEFAULTS.LOGCASE_CHANNEL_ID,
        bypdSendChannelId: data.BYPD_SEND_CHANNEL_ID || DEFAULTS.BYPD_SEND_CHANNEL_ID,
        proctorChannelId: data.PROCTOR_CHANNEL_ID || DEFAULTS.PROCTOR_CHANNEL_ID
    };
...
function checkWarnings(result) {
    ...
    const requiredFields = [
        ...
        ['logCaseChannelId', 'LOGCASE_CHANNEL_ID'],
        ['bypdSendChannelId', 'BYPD_SEND_CHANNEL_ID'],
        ['proctorChannelId', 'PROCTOR_CHANNEL_ID'],
...
async function loadSheetConfig() {
    try {
        ...
        console.log(`✅ [CONFIG] LogCase: ${views.logCaseChannelId || '?'} | Count: ${views.count.SPREADSHEET_ID ? '✅' : '?'} | Registry: ${views.registry.spreadsheetId ? '✅' : '?'}`);
        return views;
    } catch (error) {
...
function getLogCaseChannelId() {
    return views.logCaseChannelId;
}
